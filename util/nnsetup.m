...
% default
    ...
    nn.plot                             = 0;            % display training progress
% learning (convergence speed)
    ...
    nn.momentum                         = 0;            %  Momentum, faster conv, better stability, rec. values 0~0.3
    nn.scaling_learningRate             = 1;         %  Scaling factor for the learning rate (each epoch), l.r. decline, (0.99^100=0.36)
% generalization (stability)  
    nn.weightPenaltyL2                  = 0;            %  L2 regularization, weight decay, rec.val 10^-4~10^-5
    nn.nonSparsityPenalty               = 0;            %  Non sparsity penalty
    nn.sparsityTarget                   = 0.05;         %  Sparsity target
    nn.inputZeroMaskedFraction          = 0;            %  Used for Denoising AutoEncoders
    nn.dropout                          = 0;            %  Dropout level (http://www.cs.toronto.edu/~hinton/absps/dropout.pdf),0~-0.5, needs time to settle,percantage(0==off, 0.6==60% neur dropped out)
% other
