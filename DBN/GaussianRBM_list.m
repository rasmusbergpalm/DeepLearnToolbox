...
function [vhW, vb, hb, fvar, errs] = GaussianRBM_list(batchdata_list, params)

%batchdata=load(batchdata_list{1});
...
for epoch = 1:params.maxepoch
    ...
    ptot = 0;
     NBatches=0;
    for loop=1:length(batchdata_list)
        load(batchdata_list{loop});
        [n d nBatches]=size(batchdata);
       NBatches=NBatches+nBatches;
...
%             if sum(sum(abs(vhWInc)>max(max(vhW))))
%                 error('Huge fluctuation!\n')
%                 vhWInc(abs(vhWInc)>max(max(vhW)))=0;
%                 vhWInc(abs(vhWInc)<min(min(vhW)))=0;
%             end
       ...
       if params.debug     
        fprintf(1, ' p%1.2f  ',  mean(pos_hidprobs(:)) );
        fprintf(1, ' error %6.2f   mm:%.2f ',  sum(sum( (data-negdata).^2 )), momentum);
        fprintf(1, 'vh_W min %2.4f   max %2.4f ', min(min(vhW)), max(max(vhW)));
        fprintf(1, '  vh_WINC min %2.4f   max %2.4f \n', min(min(vhWInc)), max(max(vhWInc)));
       end
            %%%%%%%%%%%%%%%% END OF UPDATES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
        end
    end
