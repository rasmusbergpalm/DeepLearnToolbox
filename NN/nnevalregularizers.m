...
for i = 2 : nn.n-1
    ...
    curr_sparsity = curr_sparsity + mean(nn.p{i});
end
unitcost_sparsity = unitcost_sparsity / (nn.n-2);
curr_sparsity = curr_sparsity / (nn.n-2);

costs = [cost_prediction cost_weightL2 cost_sparsity];
total_cost = sum(costs);

fprintf('\tPercentage contributions (Prediction error, L2 regularizer, sparsity regularizer): %.1f, %.1f, %.1f;\n', 100*costs/total_cost);
fprintf('\tUnit costs of (L2, sparsity) regularizers: %f, %f;\t Current sparsity: %f\n', unitcost_weightL2, unitcost_sparsity, curr_sparsity);